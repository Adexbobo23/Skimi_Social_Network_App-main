...
def public_profile(request, username):
    ...
    return render(request, 'users/public_profile.html', {"cuser":user, 'random_users':random_users,  'user_profile': user_profile })
...
class ProfileDetailView(LoginRequiredMixin,DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Add 'random_users' context
        users = list(Profile.objects.exclude(user=self.request.user))
        if len(users) > 3:
            cnt = 5
        else:
            cnt = len(users)
